...
{
	printf ("\n\nHello, I'm Songshuai, The u-boot version is V1.0.6\n\n");
	return (0);
}
...
void OperationSelect(void)
{
	...
	printf("   5: Entr ALL LED test mode. Songshuai says Useless\n");
	printf("   6: Entr Web failsafe mode. Songshuai says Useless\n");
...
/*failsafe end!*/
	...
	printf ("\nThe Countdown remains is : %d", timer1);
	...
	while (timer1 > 0) 
	{
		...
		for (i = 0; i < 100; ++i) 
		{
			...
			{	
				...
				BootType = getc();
				if(confirm != 'n' || confirm != 'N') confirm = '3';
				if ((BootType < '0' || BootType > '6') && (BootType != '7') && (BootType != '8') && (BootType != '9'))
					BootType = '3';
				if(BootType != '3') printf("\n\rYou choosed %c\n\n", BootType);
				break;
			}
			udelay (10000);
		}
		printf("\b%d", timer1); //\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b
	}
	putc ('\n');
	if(BootType == '3') 
	{
	...
	else 
	{
		...
		case '2':
			...
			confirm = getc();
			/* If you enter Enter directly, the default is 'Y'*/
			if (confirm != 'n' || confirm != 'N') confirm = 'y';
...
void gpio_test( void )
{
	...
	for(i = 0; i < 100; i++)
	{
		...
		if(detect_wps()) break;
		j ++;
		if(j % 10 == 0) printf("\nPress WPS button to stop LED test!\n");
	}
